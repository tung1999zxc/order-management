...
const OrderManagement = () => {
    ...
    if (searchOption === "all") {
      ...
      if (
        ...
        !(
          ...
          order.shipping_unit.toLowerCase().includes(searchText.toLowerCase())
        )
      ) {
        return false;
      }
  ...
  const ftPending = orders.filter((order) => order.status === "Pending");
  const ftCompleted = orders.filter((order) => order.status === "Completed");
  const ftCanceled = orders.filter((order) => order.status === "Canceled");
      ...
      <Tabs defaultActiveKey="all">
        ...
        >
          <Row>
            ...
            <Col md={3} xl={2} style={{ marginLeft: "30px" }}>
